...
class webAgent:                                                 
	...
	raise cherrypy.HTTPRedirect(cherrypy.url() + 'mtconnect.shtml')
    ...
    def __init__(self):                                                         
        try:                                                                    
            ...
            if self.runningAgent.has_key('__south_ip') and  self.runningAgent.has_key('__south_mask'):
                #--override cfg file at startup with activation settings
                tmp2 = self.getSouthIp
                self.runningAgent['__south_ip'] = tmp2['addr']
                self.runningAgent['__south_mask'] = tmp2['mask'] 
    ...
    def agentStart(self, agentdir ):                                            
        ...
        try:                                                                    
            ...
            cli = "cd " + self.dataDir + agentdir + " ; agent run ../" + agentdir + "/" + ary[0] + " 1>/dev/null 2>/dev/null &"
            #cli = "cd " + self.dataDir + agentdir + " ; agent run ../" + agentdir + "/" + ary[0] + " >> /dev/null 2>&1 &"
            #print 'StartCommand: ',cli                                                           
            ps = subprocess.call(cli,shell=True)                                
            ct = 0                                                              
            time.sleep(2)                                                       
            while ct < 20:                                                      
    ...
    def createAgents(self):
        cli =  "ls /data/appdata/"                                 
        retCS = os.popen(cli).read()
        #print 'Machines: ' + retCS
        count = 0
        x = 1
        for x in range(1,11):
            #print x
            ...
            if retCS.find(mc) == -1:
                #extract tar
                #print 'creating agent' + mc +'...'
                loc = ' -C /data/appdata/'
                mData = mc + '/'
                tLine = "tar xvzf /home/root/machines.tar.gz "
                cliTar = tLine + mData + loc
                #print cliTar
                ret = os.popen(cliTar).read()
                #print 'created agent mc' + str(x)
                count += 1
    ...
    createAgents.exposed = True
    createAgents._cp_config = {'tools.json_out.on' : True}
    def agentList(self):                                                       
        try:
	    #print 'AgentList'                                                                    
            agentlist = {}                                                      
            ...
            for line in ps.split("\n"):                                         
                if line.find('grep') == -1 and line != "":                      
                   ...
                   ary = line.split()
                   finalSpot = len(ary) - 1                                      
                   start = ary[finalSpot].find("../")                                   
                   end = ary[finalSpot].find("/", start+3)                              
                   str = ary[finalSpot]                                                 
                   #print ary[finalSpot], 'start:',start, 'End:',end                                      
                   str = str[start+3:end]                                       
                   try:                                                         
